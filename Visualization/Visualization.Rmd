...
You can test your answer with the `mpg` __data frame__ found in ggplot2 (aka  `ggplot2::mpg`). `mpg` contains observations collected by the US Environment Protection Agency on 38 models of car. 
...
Or we could have mapped `class` to the _alpha_ aesthetic, which controls the transparency of the points. 
...
Often it is preferable to do statistical transformations before creating your plot.  In essence, you want to create a table first that is in the form you want it, and then you want to visualize it.  This is done using `dplyr`'s summary functions such as `summarize()` or `count()`.

##

The earlier chart can be created this way.  First create a table containing the data you want to visualize (counts of each cut):
...
This seems cumbersome the way that we've done it for illustrative purposes, but actually it can all be done in a single pipe:
